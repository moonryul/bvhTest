...
public class AvatarSettingScript 
{
    ...
    public AvatarSettingScript(GameObject root_game_object, GameObject[] bones) {
        this.root_game_object_ = root_game_object;
        this.bones_ = bones;
    }
 
    public Animator SettingGenericAvatar() {
        
        Transform avatar_root_transform = root_game_object_.transform.Find("Hips");
    ...
    void SetRequirePose()
    {
        foreach (Transform transform in this.bones_.Select(x=>x.transform)) { // x refers to a gameObject
    ...
    static Transform FindTransformUpwards(Transform transform, string name)
    {
        ...
        return transform; // return when transform.name == name
    }
    ...
    {
        var hinge       = FindTransformUpwards(wrist,   hinge_name); // get the transform of an ancestor of wrist whose name is hinge_name
    ...
    {
        IEnumerable<GameObject> bones_enumerator =this.bones_;
        ...
        var result = bones_enumerator.Select(x=>{
                                                SkeletonBone skeleton_bone = new SkeletonBone(); // T-pose of the skeleton
