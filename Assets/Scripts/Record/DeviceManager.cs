...
public class DeviceManager : MonoBehaviour
{
    ...
    private void Start()
    {
        ...
        {   //MJ: this component "DeviceManager" is attached to "Scroll View" gameObject under Canvas gameObject
            DeviceToggle t_toggle = Instantiate(this._toggle, Vector3.zero, Quaternion.identity); // MJ: Instantiate a Prefab: 토글 오브젝트를 생성함
            t_toggle.transform.SetParent(_contents, false); //MJ: Canvas -> Scroll View -> Viewport -> Content
              // 모든 토글은 _contents GameObject의 자식으로 있어야 하기 때문에, 생성한 오브젝트의 부모 설정을 해 줌
